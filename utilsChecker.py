...
def video2Images(
    ...
    # already written out?
    if (
        not os.path.exists(os.path.join(outputFolder, filePrefix + "_0.jpg"))
        or not skipIfRun
    ):
        if tSingleImage is not None:  # pop single image at time value
            CMD = (
                "ffmpeg -loglevel error -skip_frame nokey -y -ss "
                + str(tSingleImage)
                + " -i "
                + videoPath
            ...
            timeImageSamples = np.linspace(
                1, lengthVideo - 1, nImages
            )  # disregard first and last second
            for iFrame, t_image in enumerate(timeImageSamples):
                CMD = (
                    "ffmpeg -loglevel error -skip_frame nokey -ss "
                    + str(t_image)
                    + " -i "
                    + videoPath
                    + " -qmin 1 -q:v 1 -frames:v 1 -vf select='-eq(pict_type\,I)' "
                    + os.path.join(outputFolder, filePrefix)
                    + "_"
                    + str(iFrame)
                    + ".jpg"
                )
...
def calcIntrinsics(
    ...
    for iImage, pathName in enumerate(imageFiles):
        ...
        if imageScaleFactor != 1:
            dim = (
                int(imageScaleFactor * image.shape[1]),
                int(imageScaleFactor * image.shape[0]),
            )
            image = cv2.resize(image, dim, interpolation=cv2.INTER_AREA)
        imageSize = np.reshape(
            np.asarray(np.shape(image)[0:2]).astype(np.float64), (2, 1)
        )  # This all to be able to copy camera param dictionary
        ...
        if ret == True:
            ...
            checkerCopy["dimensions"] = meta.shape[
                ::-1
            ]  # reverses order so width is first
            ...
            corners2 = (
                corners / imageScaleFactor
            )  # Don't need subpixel refinement with findChessboardCornersSBWithMeta
            twodpoints.append(corners2)

            # Draw and display the corners
            image = cv2.drawChessboardCorners(image, meta.shape[::-1], corners2, ret)
            ...
            cv2.imwrite(
                os.path.join(
                    imageSaveDir, "intrinsicCheckerboard" + str(iImage) + ".jpg"
                ),
                image,
            )
    ...
    if len(twodpoints) < 0.5 * len(imageFiles):
        print(
            "Checkerboard not detected in at least half of intrinsic images. Re-record video."
        )
        return None
...
def computeAverageIntrinsics(session_path, trialIDs, CheckerBoardParams, nImages=25):
        ...
        if not os.path.exists(os.path.join(video_dir, "cameraIntrinsics.pickle")):
            ...
            if CamParams is None:
                saveCameraParameters(
                    os.path.join(video_dir, "cameraIntrinsics.pickle"), CamParams
                )

        else:
            CamParams = loadCameraParameters(
                os.path.join(video_dir, "cameraIntrinsics.pickle")
            )
    ...
    for v in intCompNames:
        ...
        intComp[v + "_stdPerc"] = (
            np.divide(intComp[v + "_std"], intComp[v + "_u"]) * 100
        )
    ...
    for param in params:
        CamParamsAverage[param] = np.mean(
            np.asarray([c[param] for c in CamParamList]), axis=0
        )
...
def generate3Dgrid(CheckerBoardParams):
    ...
    objectp3d = np.zeros(
        (
            1,
            CheckerBoardParams["dimensions"][0] * CheckerBoardParams["dimensions"][1],
            3,
        ),
        np.float32,
    )
    objectp3d[0, :, :2] = np.mgrid[
        0 : CheckerBoardParams["dimensions"][0], 0 : CheckerBoardParams["dimensions"][1]
    ].T.reshape(-1, 2)
...
def calcExtrinsics(
    ...
    if imageUpsampleFactor != 1:
        dim = (
            int(imageUpsampleFactor * image.shape[1]),
    ...
    ret, corners = cv2.findChessboardCorners(
        grayColor, CheckerBoardParams["dimensions"], cv2.CALIB_CB_ADAPTIVE_THRESH
    )
    ...
    if ret == True:
        ...
        # for given 2d points.
        corners2 = (
            cv2.cornerSubPix(grayColor, corners, (11, 11), (-1, -1), criteria)
            / imageUpsampleFactor
        )
    ...
    topLevelExtrinsicImageFolder = os.path.abspath(
        os.path.join(os.path.dirname(imageFileName), "../../../../CalibrationImages")
    )
    ...
    for iRet, rvec, tvec in zip(range(rets), rvecs, tvecs):
        ...
        topEdge = int(
            np.max([np.squeeze(np.min(img_points, axis=0))[1] - bufferVal, 0])
        )
        leftEdge = int(
            np.max([np.squeeze(np.min(img_points, axis=0))[0] - bufferVal, 0])
        )
        bottomEdge = int(
            np.min([np.squeeze(np.max(img_points, axis=0))[1] + bufferVal, ht])
        )
        rightEdge = int(
            np.min([np.squeeze(np.max(img_points, axis=0))[0] + bufferVal, wd])
        )
        ...
        savePath2 = os.path.join(
            os.path.dirname(imageFileName), "extrinsicCalib_soln{}.jpg".format(iRet)
        )
        ...
        camName = os.path.split(
            os.path.abspath(os.path.join(os.path.dirname(imageFileName), "../../"))
        )[1]
        ...
        savePath = os.path.join(
            topLevelExtrinsicImageFolder, "extrinsicCalib_" + fullCamName + ".jpg"
        )
...
def autoSelectExtrinsicSolution(sessionDir, keypoints2D, confidence, extrinsicsOptions):
    ...
    (
        optimalCalibrationDict[camNames[0]],
        optimalCalibrationDict[camNames[1]],
    ...
    calibrationOptionsFile = os.path.join(
        sessionDir, "Videos", "calibOptionSelections.json"
    )
    ...
    for camName in camNames:
        CamParamDict[camName] = extrinsicsOptions[camName][
            optimalCalibrationDict[camName]
        ]
    ...
    for cam, val in optimalCalibrationDict.items():
        if val == 1:
            saveFileName = os.path.join(
                sessionDir, "Videos", cam, "cameraIntrinsicsExtrinsics.pickle"
            )
...
# %%
def computeOptimalCalibrationCombination(
    keypoints2D, confidence, extrinsicsOptions, CamNames, firstCamSoln=None
):
    ...
    keypointList = [
        np.expand_dims(
            np.delete(keypoints2D[CamNames[0]], idxFaceMarkers, axis=0)[:, iFrame],
            axis=1,
        ),
        np.expand_dims(
            np.delete(keypoints2D[CamNames[1]], idxFaceMarkers, axis=0)[:, iFrame],
            axis=1,
        ),
    ]
    ...
    for iCam0 in firstCamOptions:
        for iCam1 in range(len(extrinsicsOptions[CamNames[1]])):
            ...
            CameraParamList = [
                extrinsicsOptions[CamNames[0]][iCam0],
            ...
            reprojError = calcReprojectionError(
                cameraObjList, pointsInput, points3D, normalizeError=True
            )
    ...
    if (sorted(meanReprojectionErrors)[1] - sorted(meanReprojectionErrors)[0]) / sorted(
        meanReprojectionErrors
    )[0] < 0.5:
        ...
        print(
            "Warning: not much separability between auto checker board selection options. Try moving checkerboard closer to cameras, and move it so it is not straight-on with any of the cameras."
        )
...
def orderCamerasForAutoCalDetection(extrinsicsOptions):
    ...
    sortedCams = [
        cam for _, cam in sorted(zip(rotDifs, extrinsicsOptions.keys()), reverse=True)
    ]
...
def synchronizeVideoKeypoints(
    ...
    if visualize2Dkeypoint:
        ...
        for camNum in range(nCams):
            ...
            usedInds = np.argwhere(
                np.logical_or(
                    confidenceListFilt[camNum][mkr, :] > confidenceThreshold,
            ...
            if len(usedInds) > 0:
                ax.plot(
                    usedInds,
                    keypointListFilt[camNum][mkr, usedInds, mkrXY],
                    linewidth=1,
                )
                ax.set_ylim(
                    (
                        0.9 * np.min(keypointListFilt[camNum][mkr, usedInds, mkrXY]),
                        1.1 * np.max(keypointListFilt[camNum][mkr, usedInds, mkrXY]),
                    )
                )
            else:
                ax.text(
        ...
        plt.tight_layout()
        ax.legend(
            ["unfilt", "occlusionRemoved", "filt", "used for triang"],
...
def detectHandPunchAllVideos(handPunchPositionList, sampleFreq, punchDuration=3):
    ...
    for pos in handPunchPositionList:
        ...
        for iSide in range(2):
            relPos = -np.diff(
                pos[(iSide, iSide + 2), :], axis=0
            )  # vertical position of wrist over shoulder
...
def syncHandPunch(positions, hand, maxShiftSteps=600):
    ...
    for pos in positions:
        relPos = -np.diff(
            pos[(startInd, startInd + 2), :], axis=0
        )  # vertical position of wrist over shoulder
        relVel.append(np.squeeze(np.diff(relPos)))

    corr_val, lag = cross_corr(
        relVel[1], relVel[0], multCorrGaussianStd=maxShiftSteps, visualize=False
    )
...
def getLargestBoundingBox(data, bbox, confThresh=0.6):
    ...
    badRows = np.unique(np.concatenate((rows_nonzeros_10m, rows_lowConf, rows_badFeet)))
...
def trackBoundingBox(
    ...
    try:
        ...
        ok = tracker.init(
            frame, tuple(bbox.astype(int))
        )  # bbox has to be tuple for legacy trackers, like MOSSE
    ...
    while frameNum > -1 and frameNum < nFrames:
        ...
        # Find person closest to tracked bounding box, and fill their keypoint data
        keyBoxes = [box[frameNum] for box in allBoxes]
        iPerson, bboxKey, samePerson = findClosestBox(
            bbox, keyBoxes, imageSize
        )  # it may be better to find the one closest to bboxKey, but then it wouldn't be leveraging
        ...
        if updateCounter > 20:
            ...
            try:
                ...
                ok = tracker.init(
                    frame, tuple(bboxKey.astype(int))
                )  # bbox has to be tuple for legacy trackers, like MOSSE
...
def removeOccludedSide(key2D, confidence, mkrInds, confThresh, visualize=False):
    ...
    # Parameters
    confDif = 0.2  # the difference in mean confidence between R and L sided markers. If dif > confDif, must be occluded
    ...
    if visualize:
        ...
        plt.plot(dConf, color=(0.5, 0.5, 0.5))
        plt.legend(["rightConfidence", "leftConfidence", "dConf"])
        plt.xlabel("frame")
        plt.ylabel("mean confidence")

    rOccluded = np.where(
        np.logical_and(np.less(dConf, confDif), np.less(rConf, confThresh))
    )[0]
    lOccluded = np.where(
        np.logical_and(np.greater(dConf, confDif), np.less(lConf, confThresh))
    )[0]
    ...
    def zeroPad(occInds, nSamps):
        ...
        if occInds[-1] == nSamps - 1:
            ...
            if not any(dInds > 1):
                ...
                zeroInds = np.concatenate(
                    (
                        zeroInds,
                        np.arange(occInds_init[lastGap[0]], occInds_init[-1] + 1),
                    )
                )
    ...
    for mkr in lMkrs:
        if np.count_nonzero(confidence_out[mkr, :] > 0) > 3:
            lNan = np.intersect1d(
                lOccluded,
                np.arange(
                    np.argwhere(confidence_out[mkr, :] > 0)[1],
                    np.argwhere(confidence_out[mkr, :] > 0)[-1],
                ),
            )  # don't add nans to 0 pad
        else:
...
def getVertVelocity(key2D):
    ...
    if (
        not np.max(np.abs(vertVelTotal)) == 0
    ):  # only if markers were found, otherwise div by 0
...
def getMarkerSpeed(
    key2D, idxMkrs=[0], confidence=None, confThresh=0.2, averageVels=False
):
    ...
    vertVel = np.abs(
        np.linalg.norm(
            np.diff(
                ...
                axis=1,
                n=diffOrder,
                append=np.tile(key2D[:, [-1], :], (1, diffOrder, 1)),
            ),
            axis=2,
        )
    )
    ...
    for i in range(25):
        for j in range(2):
            key2D_out[i, :, j] = np.apply_along_axis(
                lambda x: gaussian_filter1d(x, sdKernel), arr=key2D_out[i, :, j], axis=0
            )
    return key2D_out
...
def filter3DPointsButterworth(points3D, filtFreq, sampleFreq, order=4):
    ...
    if wn > 1:
        print(
            "You tried to filter "
            + str(sampleFreq)
            + " signal with cutoff freq of "
            + str(filtFreq)
            + ", which is above the Nyquist Frequency. Will filter at "
            + str(sampleFreq / 2)
            + "instead."
        )
        wn = 0.99
...
# %%
def clean2Dkeypoints(
    key2D, confidence, confidenceThreshold=0.5, nCams=2, linearInterp=False
):
    key2D_out = np.copy(key2D)
    confidence_out = np.copy(confidence)
        ...
        for j in range(2):
            if np.isnan(key2D_out[i, :, j]).all():  # only nans
                ...
                if linearInterp:
                    ...
                    key2D_out[i, nans, j] = np.interp(
                        x(nans), x(~nans), key2D_out[i, ~nans, j]
                    )
                else:
...
def cross_corr_multiple_timeseries(
    Y1, Y2, multCorrGaussianStd=None, dataForReproj=None, visualize=False
):
...
def writeTRCfrom3DKeypoints(
    ...
    with open(pathOutputFile, "w") as f:
        numpy2TRC(f, keypoints3D_res_sel, keypointNames_sel, fc=frameRate, units="m")
...
def findOverlap(confidenceList, markers4VertVel):
    ...
    confMean = [np.mean(cMat[markers4VertVel, :], axis=0) for cMat in c_confidenceList]
    ...
    confThresh = 0.5 * np.nanmax(minConfidence)
    ...
    if unConfidentInds.shape[0] > 0:
        ...
        if unConfidentInds.shape[0] > 1:
            for otherStretch in range(unConfidentInds.shape[0] - 1):
                stretches.append(
                    np.arange(
                        unConfidentInds[otherStretch] + 1,
                        unConfidentInds[otherStretch + 1] + 1,
                    )
                )
        # Last stretch (ie, between last unconfident index and last element).
        stretches.append(np.arange(unConfidentInds[-1] + 1, overlapInds.shape[1]))
...
def popNeutralPoseImages(
    ...
    for iCam, cam in enumerate(cameras2Use):
        ...
        videoPath = os.path.join(
            cameraDirectories_selectedCams[cam],
            "InputMedia",
            "neutral",
            "{}_rotated.avi".format(trial_id),
        )

        imagePath = video2Images(
            videoPath,
            tSingleImage=tSingleImage,
        ...
        if writeVideo:
            ...
            ffmpegCmd = (
                "ffmpeg -loglevel error -r 0.01 -loop 1 -i "
                + imagePath
