...
def postCalibrationOptions(session_path, session_id, overwrite=False):
    ...
    if trial["meta"] is None or overwrite == True:
        calibOptionsJsonPath = os.path.join(
            session_path, "Videos", "calibOptionSelections.json"
        )
        f = open(calibOptionsJsonPath)
        calibOptionsJson = json.load(f)
        f.close()
        data = {"meta": json.dumps({"calibration": calibOptionsJson})}
...
def downloadVideosFromServer(
    ...
    if not benchmark:
        if not os.path.exists(
            os.path.join(session_path, "Videos", "mappingCamDevice.pickle")
        ):
            mappingCamDevice = {}
            ...
            with open(
                os.path.join(session_path, "Videos", "mappingCamDevice.pickle"), "wb"
            ) as handle:
                pickle.dump(mappingCamDevice, handle)
        else:
            with open(
                os.path.join(session_path, "Videos", "mappingCamDevice.pickle"), "rb"
            ) as handle:
            ...
            for video in trial["videos"]:
                ...
                videoDir = os.path.join(
                    session_path, "Videos", "Cam{}".format(k), "InputMedia", trial_name
                )
        ...
        if not os.path.exists(sessionYamlPath) or isStaticPose or isCalibration:
            if isCalibration:  # subject parameters won't be entered yet
                session_desc = getMetadataFromServer(
                    session_id, justCheckerParams=isCalibration
                )
            ...
            session_desc["iphoneModel"] = {
                "Cam" + str(i): phoneModel[i] for i in range(len(phoneModel))
            }
...
def deleteCalibrationFiles(session_path):
    ...
    if len(camDirs) > 1 and os.path.exists(os.path.join(camDirs[0], "InputMedia")):
        ...
        trialNames = [
            tName
            for tName in dirContents
            if os.path.isdir(os.path.join(inputDir, tName))
        ]
...
def deleteStaticFiles(session_path, staticTrialName="neutral"):
    ...
    if os.path.exists(openSimDir):
        shutil.rmtree(
            openSimDir
        )  # Static will be the first opensim data saved, so this is safe
...
def switchCalibrationForCamera(cam, trial_id, session_path):
    ...
    dest = os.path.join(
        session_path, "CalibrationImages", "extrinsicCalib" + cam + ".jpg"
    )
...
def getMetadataFromServer(session_id, justCheckerParams=False):
    defaultMetadataPath = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "defaultSessionMetadata.yaml"
    )
    ...
    if session["meta"] is not None:
        if not justCheckerParams:
            ...
            if "subject" in session["meta"]:
                ...
                if "settings" in session["meta"]:
                    try:
                        session_desc["openSimModel"] = session["meta"]["settings"][
                            "openSimModel"
                        ]
                    except:
                        session_desc["openSimModel"] = "LaiUhlrich2022"
                    try:
                        session_desc["augmentermodel"] = session["meta"]["settings"][
                            "augmentermodel"
                        ]
                    except:
        ...
        if (
            "sessionWithCalibration" in session["meta"]
            and "checkerboard" not in session["meta"]
        ):
            newSessionId = session["meta"]["sessionWithCalibration"]["id"]
            session = getSessionJson(newSessionId)

        session_desc["checkerBoard"]["squareSideLength_mm"] = float(
            session["meta"]["checkerboard"]["square_size"]
        )
        session_desc["checkerBoard"]["black2BlackCornersWidth_n"] = int(
            session["meta"]["checkerboard"]["cols"]
        )
        session_desc["checkerBoard"]["black2BlackCornersHeight_n"] = int(
            session["meta"]["checkerboard"]["rows"]
        )
        session_desc["checkerBoard"]["placement"] = session["meta"]["checkerboard"][
            "placement"
        ]
        ...
        print(
            "Couldn"
            "t find session metadata in API, using default metadata. May be issues."
        )
...
def writeCalibrationOptionsToAPI(
    session_path, session_id, calibration_id=None, trialName=None
):
    ...
    for camDir in camDirs:
        ...
        for i in range(2):
            filePath = os.path.join(
                calibDir, "cameraIntrinsicsExtrinsics_soln{}.pickle".format(i)
            )
...
def getCalibration(
    session_id, session_path, trial_type="dynamic", getCalibrationOptions=False
):
    ...
    calibrationOptions = downloadAndSwitchCalibrationFromDjango(
        session_id,
        session_path,
...
def downloadAndSwitchCalibrationFromDjango(
    session_id, session_path, calibTrialID=None, getCalibrationOptions=False
):
    if calibTrialID == None:
        calibTrialID = getCalibrationTrialID(session_id)
    ...
    calibURLs = {
        t["device_id"]: t["media"]
        for t in trial["results"]
        if t["tag"] == "calibration_parameters_options"
    }

    if (
        "meta" in trial.keys()
        and trial["meta"] is not None
        and "calibration" in trial["meta"].keys()
    ):
...
def changeSessionMetadata(session_ids, newMetaDict):
    for session_id in session_ids:
        ...
        mapping_metadata = {"mass": "mass_kg", "height": "height_m"}
        addedKey = {}
        ...
        for newMeta in newMetaDict:
            if not newMeta in addedKey:
                print(
                    "Could not find {} in existing metadata, trying to add it.".format(
                        newMeta
                    )
                )
                settings_fields = [
                ...
                if newMeta in settings_fields:
                    ...
                    print(
                        "Added {} to settings in metadata".format(newMetaDict[newMeta])
                    )
                else:
                    print(
                        "Could not add {} to the metadata; not recognized".format(
                            newMetaDict[newMeta]
                        )
                    )
...
def makeSessionPublic(session_id, publicStatus=True):
    ...
    r = requests.patch(
        session_url, data=data, headers={"Authorization": "Token {}".format(API_TOKEN)}
    )
...
def postMotionData(trial_id, session_path, trial_name=None, isNeutral=False):
    ...
    if isNeutral:
        ...
        ikPath = os.path.join(
            session_path, "OpenSimData", "Kinematics", trial_id + ".mot"
        )
...
def getMotionData(trial_id, session_path, simplePath=False):
    ...
    if "ik_results" in resultTags:
        markerFolder = os.path.join(
            session_path, "MarkerData", "PostAugmentation", trial_name
        )
        if simplePath:
...
def downloadAndZipSession(
    session_id,
    deleteFolderWhenZipped=True,
    isDocker=True,
    writeToDjango=False,
    justDownload=False,
    data_dir=None,
    ...
    dynamic_ids = [
        ...
        if (t["name"] != "calibration" and t["name"] != "neutral")
    ]

    # Calibration
    downloadVideosFromServer(
        session_id, calib_id, isDocker=isDocker, isCalibration=True, isStaticPose=False
    )
    ...
    if not justDownload:
        ...
        if writeToDjango:
            postFileToTrial(
                session_zip, dynamic_ids[-1], tag="session_zip", device_id="all"
            )
...
def getIK(storage_file, joints, degrees=False):
    ...
    QsFilt = pd.concat(
        [pd.DataFrame(data=data["time"], columns=["time"]), output], axis=1
    )
...
def getSubjectNumber(subjectName):
    subjects = requests.get(
        API_URL + "subjects/", headers={"Authorization": "Token {}".format(API_TOKEN)}
    ).json()
    sNum = [s["id"] for s in subjects if s["name"] == subjectName]
    if len(sNum) > 1:
        print(
            len(sNum)
            + " subjects with the name "
            + subjectName
