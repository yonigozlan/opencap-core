...
def runScaleTool(
    ...
    if "Lai" in scaledModelName or "Rajagopal" in scaledModelName:
        if "Mocap" in setupFileName:
            ...
            markerSetFileName = "RajagopalModified2016_markers_mmpose_anatomical.xml"
...
# %%
def runOpenPosePKTool(
    pathGenericSetupFile, pathModel, pathIKFile, timeRange, pathOutputFolder
):
    markers = [
        "Neck",
        "RShoulder",
    ...
    _, IKFileName = os.path.split(pathIKFile)
    IKFileName = IKFileName[:-4]
    pathOutputSetup = os.path.join(pathOutputFolder, "Setup_PK_" + IKFileName + ".xml")
    ...
    for CleanUp in glob.glob(pathOutputFolder + "/*.*"):
        if (not CleanUp.endswith(".xml")) and (not CleanUp.endswith("_pos.sto")):
            os.remove(CleanUp)


# %%
def exportTRCFromOpenPosePK(pathPKFolder, fileName):
    from utils import numpy2TRC, storage2df
    ...
    pathPKMarker = os.path.join(
        pathPKFolder, "PK_" + fileName + "_PointKinematics_" + markers[0] + "_pos.sto"
    )
    ...
    for marker in markers[1:]:
        pathPKMarker = os.path.join(
            pathPKFolder, "PK_" + fileName + "_PointKinematics_" + marker + "_pos.sto"
        )
...
def compareTRCAndForcesTime(pathTRC, pathForces):
    ...
    assert len(forceTime) == nForceSamples, "Force and IK time ranges do not match"
