...
def main(
    ...
    if genericFolderNames:
        ...
        if not markerDataFolderNameSuffix is None:
            markerDataFolderName = os.path.join(
                markerDataFolderName, markerDataFolderNameSuffix
            )
    preAugmentationDir = os.path.join(
        sessionDir, markerDataFolderName, "PreAugmentation"
    )
    ...
    pathOutputFiles = {}
    if benchmark:
        pathOutputFiles[trialName] = os.path.join(
            preAugmentationDir, trialName + ".trc"
        )
    else:
        pathOutputFiles[trialName] = os.path.join(preAugmentationDir, trial_id + ".trc")
    ...
    if scaleModel and calibrationOptions is not None and alternateExtrinsics is None:
        ...
        CamParamDict = autoSelectExtrinsicSolution(
            sessionDir, keypoints2D, confidence, calibrationOptions
        )
    ...
    if genericFolderNames:
        postAugmentationDir = os.path.join(
            sessionDir, markerDataFolderName, "PostAugmentation"
        )
    else:
        postAugmentationDir = os.path.join(
            sessionDir,
            markerDataFolderName,
            "PostAugmentation_{}".format(augmenterModel),
        )

    # Get augmenter model.
    augmenterModelName = sessionMetadata["markerAugmentationSettings"][
    ...
    if runMarkerAugmentation:
        ...
        try:
            vertical_offset = augmentTRC(
                ...
                sessionMetadata["height_m"],
                pathAugmentedOutputFiles[trialName],
                augmenterDir,
                augmenterModelName=augmenterModelName,
    ...
    if runOpenSimPipeline:
        ...
        if genericFolderNames:
            ...
            openSimFolderName = os.path.join("OpenSimData", poseDetector + suff_pd)
            if not markerDataFolderNameSuffix is None:
                openSimFolderName = os.path.join(
                    openSimFolderName, markerDataFolderNameSuffix
                )
        ...
        outputJsonVisDir = os.path.join(sessionDir, "VisualizerJsons", trialName)
        os.makedirs(outputJsonVisDir, exist_ok=True)
        outputJsonVisPath = os.path.join(outputJsonVisDir, trialName + ".json")
    ...
    if not extrinsicsTrial:
        ...
        pathSettings = os.path.join(
            postAugmentationDir, "Settings_" + trial_id + ".yaml"
        )
        settings = {
            "poseDetector": poseDetector,
            "resolutionPoseDetection": resolutionPoseDetection,
